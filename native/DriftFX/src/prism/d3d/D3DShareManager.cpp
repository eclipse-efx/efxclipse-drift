...
void D3DShareManager::doPresent(RenderTarget* target) {
	...
	GLuint glTex = target->GetGLTexture();
	...
	glFramebufferTexture(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, glTex, 0);

	//glNamedFramebufferTexture(fbos[0], GL_COLOR_ATTACHMENT0, glTex, 0);

	GLuint blub;
	glGenTextures(1, &blub);

	wglDXSetResourceShareHandleNV(dxTex->GetTexture(), dxTex->GetShareHandle());
	HANDLE shareObject = wglDXRegisterObjectNV(shareDevice, dxTex->GetTexture(), blub, GL_TEXTURE_2D, WGL_ACCESS_READ_WRITE_NV);
	...
	glFramebufferTexture(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, glTex, 0);
